...
Requires:       weston-eglstream-libs%{?_isa} = %{version}-%{release}

%description
Weston is the reference wayland compositor that can run on KMS, under X11
or under another compositor. This build uses EGLStreams instead of GBM for
buffer management. Use with the proprietary NVIDIA drivers.


%package        libs
Summary:        Weston compositor libraries
Conflicts:      weston-libs
...
License:        MIT
Conflicts:      weston-devel
Requires:       weston-eglstream%{?_isa} = %{version}-%{release}
Requires:       weston-eglstream-libs%{?_isa} = %{version}-%{release}
